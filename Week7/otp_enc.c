...
int main (int argc, char* argv[]){
   ...
   //IO Parameters
   int plainfd, keyfd, plainfsize, keyfsize, nProgress, nTemp;
   ...
   plainfsize = lseek(plainfd, 0, SEEK_END);
   lseek(plainfd, 0, 0);
   ...
   //Check if key file size is smaller than plaintext file size
   if(keyfsize < plainfsize){
      fprintf(stderr,"Client: Key file size too small for plaintext file\n");
      close(plainfd);
      close(keyfd);
      return 1;
   }
   ...
   write(sockfd, &plainfsize, sizeof(int)); 
   ...
   nProgress = 0;
   while(nProgress < plainfsize){
//      printf("nProgress = %i, plainfsize = %i\n", nProgress, plainfsize);

      //Zero buffer
      bzero(buffer,BUF_SIZE);

      //Get buffer full of text from file 
      n = read(plainfd, buffer, BUF_SIZE);
...
//      printf("Client: Sending message \"%s\"\n", buffer);
	 ...
	 break;
      ...
      //Read returned plaintext from server
