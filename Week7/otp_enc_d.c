...
int main (int argc, char* argv[]){
   ...
   while(strcmp(buffer, "quit")){
	 ...
	 //Read in expected plaintext filesize
	 read(newsockfd, &plainfsize, sizeof(int));
//	 printf("Server: File size received %i\n", plainfsize);
	 //Return expected plaintext filesize for validation
	 write(newsockfd, &plainfsize, sizeof(int));
	 childProcess(newsockfd, plainfsize);
	 exit(0);
...
void childProcess(int newsockfd, int plainfsize){
   ...
   while(nProgress < plainfsize){
      ...
      //Read plaintext chunk from client
...
//      printf("Server: Received message %s\n", buffer);
      ...
      //Write received plaintext back to client
...
//      printf("Server Received: Key %s\n", keybuf);
      ...
      //if Read in key chunk < plaintext chunk, can't encode; stop listening 
      if (nTemp < n){
	 ...
	 //Add key value (in range 0-27) to buffer[i]
